...
import { sendGeminiRequest } from "../../Utills/getResponseFromAI"; // Adjust the path as needed
import axios from "axios"; // Add this import
// Destructure `categories` directly from the props
const AIQuizGenerator = ({ categories = [] }) => {
  ...
  ]; // Use the `categories` prop directly for suggestions // Add a default empty array to `categories` if it might be undefined
...
- category: The subject/topic of the quiz
...
const parsedQuiz = JSON.parse(cleanedResponse);
  ...
  const saveQuiz = async () => {
    ...
    // You may want to get categoryId, duration, image from elsewhere in your app
    // For now, set dummy values or extend UI to collect them
    const categoryId = null; // Replace with actual categoryId if available
    const duration = 10; // Default duration in minutes
    const image = null; // Replace with actual image if available
    ...
    const quizData = {
      ...
      category: generatedQuiz.title, // Use title as category name
      categoryId,
        ...
        <div className="mt-8 border-t pt-6">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              ...
              <span
                className={`px-3 py-1 rounded-full text-sm font-medium ${difficultyOptions.find(
